...
import time
import pandas as pd
from .rmsa_env2 import RMSAEnv2
from .optical_network_env import OpticalNetworkEnv


class DeepRMSAEnv2(RMSAEnv2):
    ...
    def observation(self):
        ...
        for idp, path in enumerate(self.k_shortest_paths[self.service.source, self.service.destination]): 
            ...
            for idband in range(self.bands):        #including bands
                ...
                idx, values, lengths = DeepRMSAEnv2.rle(available_slots)
    ...
    def reward(self):
        old_fragmentation = self.spectrum_fragmentation
        if self.service.accepted:
            self.get_available_blocks(self.path, self.band)
            if self.spectrum_fragmentation < old_fragmentation:
                return 5
            else:
                return 1
        else:
...
def shortest_path_first_fit(env: DeepRMSAEnv2) -> int:
...
def shortest_available_path_first_fit(env: DeepRMSAEnv2) -> int:
