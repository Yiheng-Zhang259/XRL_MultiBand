...
class RMSAEnv2(OpticalNetworkEnv):
    ...
    def _update_link_stats(self, node1: str, node2: str):
        ...
        if self.current_time > 0:
            ...
            if np.sum(slot_allocation) > 0:
                initial_indices, values, lengths = RMSAEnv2.rle(slot_allocation)
                ...
                if len(used_blocks) > 1:
                    ...
                    internal_idx, internal_values, internal_lengths = RMSAEnv2.rle(
    ...
    def get_available_blocks(self, path, idb): # mb included
        ...
        #### add for final band
        # total number of free FSUs
        free_FSUs = np.sum(available_slots)
        ...
        # getting the blocks
        initial_indices, values, lengths = RMSAEnv2.rle(available_slots)
        ...
        size_of_largest_block = 0
        for i in available_indices[0]:
            if lengths[i] > size_of_largest_block:
                size_of_largest_block = lengths[i]
        
        # Calculate spectrum fragmentation
        self.spectrum_fragmentation = 1 - size_of_largest_block/free_FSUs
    ...
    def _get_network_compactness(self):
        ...
        for n1, n2 in self.topology.edges():
            ...
            initial_indices, values, lengths = \
                RMSAEnv2.rle(self.topology.graph['available_slots'][self.topology[n1][n2]['index'], :])
            used_blocks = [i for i, x in enumerate(values) if x == 0]
            if len(used_blocks) > 1:
                lambda_min = initial_indices[used_blocks[0]]
                lambda_max = initial_indices[used_blocks[-1]] + lengths[used_blocks[-1]]
